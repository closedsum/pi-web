...
test("all project resource loaders and reloads enforce project trust", async () => {
  const rpcSource = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const modelsSource = (await readFile(new URL("../app/api/models/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const skillsSource = (await readFile(new URL("./skills-service.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const skillsInstallSource = (await readFile(new URL("../app/api/skills/install/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const pluginsSource = (await readFile(new URL("../app/api/plugins/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
...
test("the trust API invalidates cached models and restricted runtimes", async () => {
  const source = (await readFile(new URL("../app/api/project-trust/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const rpcSource = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
