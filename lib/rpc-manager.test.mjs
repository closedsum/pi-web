...
test("get_tools preserves the SDK tool definition fields", async () => {
  const source = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
...
test("built-in subagents persist their selected resource policy", async () => {
  const source = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const subagentSource = (await readFile(new URL("./subagent-runtime.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
...
test("running snapshots expose sessions with suppressed completion notifications", async () => {
  const source = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const runningRouteSource = (await readFile(new URL("../app/api/agent/running/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const sessionsRouteSource = (await readFile(new URL("../app/api/sessions/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
...
test("RPC session startup opens an existing session file only once and trusts its cwd", async () => {
  const source = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const routeSource = (await readFile(new URL("../app/api/agent/[id]/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const eventRouteSource = (await readFile(new URL("../app/api/agent/[id]/events/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const autoNameRouteSource = (await readFile(new URL("../app/api/sessions/[id]/auto-name/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  const source = (await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const deleteRouteSource = (await readFile(new URL("../app/api/sessions/[id]/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const trustRouteSource = (await readFile(new URL("../app/api/project-trust/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
...
test("new-session route applies model scope during construction instead of follow-up commands", async () => {
  const source = (await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
...
test("prompt routes mark only preflight failures as rejected", async () => {
  const existingRoute = (await readFile(new URL("../app/api/agent/[id]/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
  const newRoute = (await readFile(new URL("../app/api/agent/new/route.ts", import.meta.url), "utf8")).replace(/\r\n/g, "\n");
