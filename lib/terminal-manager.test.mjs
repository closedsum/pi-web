...
// Transpiles terminal-manager.ts and runs it in a fresh context with a fake node-pty.
// nodePty() returns the module (or throws, like a failed native load); env and
// platform stand in for process's; timers default to the real ones.
async function loadTerminalManager({ nodePty, env = process.env, platform = process.platform,
  setTimeout: setTimer = setTimeout, clearTimeout: clearTimer = clearTimeout, console: log = console } = {}) {
  ...
  const fakeRequire = (id) => id === "node-pty" ? nodePty() : id === "./paths" ? paths : require(id);
  const exports = {};
  runInNewContext(outputText, { exports, process: { ...process, env, once() {}, platform }, require: fakeRequire,
    setTimeout: setTimer, clearTimeout: clearTimer, console: log });
  return exports;
}

test("native module load failures are deferred until creation and include repair instructions", async () => {
  const exports = await loadTerminalManager({ nodePty: () => { throw new Error("Cannot find module pty.node"); } });
...
function recordingPty(spawnCalls) {
...
test("shell environment defaults to UTF-8 locale when the host has none", async () => {
  ...
  const exports = await loadTerminalManager({ nodePty: recordingPty(spawnCalls), env: envWithoutLocale });
...
test("shell environment preserves a case-insensitive Windows locale", async () => {
  ...
  const exports = await loadTerminalManager({ nodePty: recordingPty(spawnCalls), env: envWithLocale });
  exports.createTerminal(process.cwd(), 80, 24);
  assert.equal(spawnCalls.length, 1);
...
async function loadWithFakePty(platform, kill) {
  const fakePty = { pid: 1, onData() {}, onExit() {}, write() {}, resize() {}, kill };
  const timers = [];
  const warnings = [];
  const exports = await loadTerminalManager({ nodePty: () => ({ spawn: () => fakePty }), platform,
    setTimeout: (fn) => { timers.push(fn); return { unref() {} }; }, clearTimeout() {},
    console: { ...console, warn: (...args) => warnings.push(args.join(" ")) } });
  return { exports, warnings, runTimers: () => timers.splice(0).forEach((fn) => fn()) };
}

test("Windows kills the PTY without a signal, which node-pty rejects there", async () => {
  const signals = [];
  const { exports, runTimers } = await loadWithFakePty("win32", (signal) => {
    if (signal !== undefined) throw new Error("Signals not supported on windows.");
    signals.push(signal);
  });
  exports.killTerminal(exports.createTerminal(process.cwd(), 80, 24));
  exports.killTerminal(exports.createTerminal(process.cwd(), 80, 24), true);
  runTimers();  // a still-running shell must not be escalated with a signal either
  assert.deepEqual(signals, [undefined, undefined]);
});

test("Windows reports a PTY that is still running after kill(), without signalling it", async () => {
  const { exports, warnings, runTimers } = await loadWithFakePty("win32", () => {});
  exports.killTerminal(exports.createTerminal(process.cwd(), 80, 24));
  exports.killTerminal(exports.createTerminal(process.cwd(), 80, 24), true);
  runTimers();
  assert.equal(warnings.length, 2);
  assert.match(warnings[0], /still running .*after kill\(\)/);
});

test("POSIX close sends the default signal and escalates to SIGKILL for a shell that stays up", async () => {
  const signals = [];
  const { exports, runTimers } = await loadWithFakePty("linux", (signal) => signals.push(signal));
  exports.killTerminal(exports.createTerminal(process.cwd(), 80, 24));
  exports.killTerminal(exports.createTerminal(process.cwd(), 80, 24), true);
  runTimers();
  assert.deepEqual(signals, [undefined, "SIGKILL", "SIGKILL"]);
});
...
test("native PTY starts after install and repeated creation reuses the same workspace process", (t) => {
  ...
  assert.equal(typeof record.pty.pid, "number");
