...
const MODELS = [
  ...
  { id: "gpt-6-sol", provider: "acme-gateway-openai", name: "GPT-6 (Acme)" },
];
...
test("accepts provider-qualified and unique bare exact model ids", async () => {
  ...
  const unique = await resolveVisibleModels(runtime, ["gpt-6-sol"]);
  assert.deepEqual(refs(unique), ["acme-gateway-openai/gpt-6-sol"]);
});
...
test("uses the saved default without creating a synthetic scope when unconfigured", async () => {
  ...
  const initial = selectInitialModelScope(scope, {
    defaultModel: { provider: "acme-gateway-openai", modelId: "gpt-6-sol" },
  });

  assert.equal(`${initial.model.provider}/${initial.model.id}`, "acme-gateway-openai/gpt-6-sol");
