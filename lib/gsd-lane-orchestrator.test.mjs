...
test("dispatch guard with the default cap of 1 re-allows dispatch after the cooldown", async (t) => {
  // Mocked Date (enabled before construction) keeps the cooldown boundary
  // deterministic with no wall-clock sleeps.
  t.mock.timers.enable({ apis: ["Date"], now: Date.parse("2026-01-01T00:00:00Z") });
  // maxDispatches is left to the default merge; a gsdBinDir that cannot exist
  // keeps defaults.json out of it so the shipped DEFAULT_BUDGETS value applies.
  const missingBin = join(tmpdir(), `no-gsd-bin-${randomUUID()}`);
  const { toolCallHandlers } = await buildExtensionWithToolCallCapture({
    gsdBinDir: missingBin,
