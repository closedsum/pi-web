...
test("dispatch guard with the default cap of 1 re-allows dispatch after the cooldown", async () => {
  const { toolCallHandlers } = await buildExtensionWithToolCallCapture({
    budgets: { maxDispatches: DEFAULT_BUDGETS.maxDispatches, dispatchCooldownMs: 20 },
  });
  const handler = toolCallHandlers[0];
  ...
  assert.equal((await handler(makeToolCallEvent("DispatchLane")))?.block, true, "2nd dispatch within cooldown blocks");
  await new Promise((r) => setTimeout(r, 60));
  assert.equal(await handler(makeToolCallEvent("DispatchLane")), undefined, "dispatch passes again after cooldown");
});
