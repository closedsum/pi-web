...
test("resolveModelDisplayName maps short names to display names", () => {
  assert.equal(resolveModelDisplayName("sol"), "gpt-5.6-sol");
...
test("resolveModelDisplayName passes through already-full names", () => {
  assert.equal(resolveModelDisplayName("gpt-5.6-sol"), "gpt-5.6-sol");
...
test("every family in config is reachable via at least one known model name", () => {
  const knownModels = ["opus5", "sonnet5", "haiku", "fable", "sol", "terra", "luna", "astra", "gpt-5.6-sol", "qwen38", "qwen3.8-max", "muse", "muse-spark-1.3", "MiniMax-M3"];
